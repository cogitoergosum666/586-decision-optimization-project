...
- The current script uses the shared cleaning and time-split utilities in `loan_default_data.py`, so later models can be compared on the same dataset definition.
...
def main() -> None:
    parser = argparse.ArgumentParser(description="Run full-data logistic regression for LendingClub default risk.")
    ...
    append_results(results_path, input_path, train, valid, test, results)
    print(f"Appended logistic regression results to {results_path}")
